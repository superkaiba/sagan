 ...
 *
 * Reads DATABASE_URL_DIRECT from the repo's .env file directly so we don't
 * depend on shell-side env loading.
 */
...
async function main() {
  try {
    await sql.begin(async (tx) => {
      const exp = await tx<{ id: string; status: string; title: string }[]>`
        SELECT id, status, title FROM experiments WHERE id = ${EXPERIMENT_ID}
      `;
      if (exp.length === 0) throw new Error(`experiment ${EXPERIMENT_ID} not found`);
      console.log('Found experiment:', exp[0]);

      const comment = await tx<{ id: string; resolved_at: Date | null }[]>`
        SELECT id, resolved_at FROM comments WHERE id = ${COMMENT_ID}
      `;
      if (comment.length === 0) throw new Error(`comment ${COMMENT_ID} not found`);
      if (comment[0].resolved_at) {
        console.warn('Comment was already resolved — re-resolving anyway');
      }

      const expUpdate = await tx`
        UPDATE experiments
           SET body = ${NEW_BODY},
               compute_size = 'large',
               updated_at = now()
         WHERE id = ${EXPERIMENT_ID}
         RETURNING id, status, compute_size, updated_at
      `;
      console.log('Updated experiment row:', expUpdate[0]);

      const cmtUpdate = await tx`
        UPDATE comments
           SET resolved_at = now(),
               ...
               updated_at = now()
         WHERE id = ${COMMENT_ID}
         RETURNING id, resolved_at, agent_run_id
      `;
      console.log('Updated comment row:', cmtUpdate[0]);
    });
